...
func (c *Compiler) validateWorkflowData(workflowData *WorkflowData, markdownPath string) error {
	workflowPermissions, err := c.validateWorkflowDataCore(workflowData, markdownPath)
	if err != nil {
		return err
	}

	if err := c.validateWorkflowDataPermissions(workflowData, markdownPath, workflowPermissions); err != nil {
		return err
	}

	return c.validateWorkflowDataDispatchAndResources(workflowData, markdownPath)
}
